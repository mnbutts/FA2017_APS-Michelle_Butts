...
//Add New Panel for topheader Setups
	$wp_customize->add_panel(
		...
		array(
			...
			'theme_supports' => '',
			'title' => __( 'Header Settings', 'wp-store-child' ),
			'description' => __( 'Setup header of the site.', 'wp-store-child' ),
			)
		);
		...
		$wp_customize->add_section(
			...
			array(
				'title'           =>      __('Ticker Setting', 'wp-store-child'),
				'priority'        =>      '2',
			...
			$wp_customize->add_control(
				new Wp_store_WP_Customize_Switch_Control(
					...
					array(
						...
						'label'         =>      __('Enable Ticker in Header', 'wp-store-child'),
						'type'          =>      'switch',
						)
					)
				);
			...
			$wp_customize->add_setting(
				...
				array(
					'default'       =>      __('Latest','wp-store-child'),
			...
			$wp_customize->add_control(
				...
				array(
					'section'       =>      'wp_store_header_setting_ticker',
					'label'         =>      __('Ticker Title', 'wp-store-child'),
					'type'          =>      'text'
					)
				);
			...
			$wp_customize->add_control(
				...
				array(
					'type'	=> 'select',
					'label' => __('Select a category to show in ticker','wp-store-child'),
	...
	// Header Search option
		$wp_customize->add_section(
			...
			array(
				'title' => __('Header Search Setting','wp-store-child'),
				'priority' => '25',
				'panel' => 'wp_store_header_setting'
				)
			);
			...
			$wp_customize->add_control(
				new Wp_store_WP_Customize_Switch_Control(
					...
					array(
						'type' => 'switch',
						'label' => __('Check to Show Search in Header','wp-store-child'),
			...
			//Search Box Placeholder
	   	    $wp_customize->add_setting(
		    	...
		    	array(
					'default' => __('Search...','wp-store-child'),
		        	'sanitize_callback' => 'wp_store_sanitize_text',
		        	'transport' => 'postMessage',
					)
		    	);
		    ...
		    $wp_customize->add_control(
		    	'wp_store_header_setting_search_placeholder',
		    	array(
		    		'label' 	=>	__('Enter Search Placeholder','wp-store-child'),
			        'type' => 'text',
			        'section' => 'wp_store_header_setting_search',
    ...
    $wp_customize->add_section(
    	...
    	array(
			'title' => __('Call-To','wp-store-child'),
			'priority' => '10',
			'panel' => 'wp_store_header_setting'
			)
    	);
		...
		$wp_customize->add_control(
			...
			array(
				'type' => 'textarea',
				'label' => __('Call To Content','wp-store-child'),
				'description' => __('Enter text or HTML for call to action','wp-store-child'),
	    ...
	    //logo Alignment
	   	$wp_customize->add_section(
	   		...
	   		array(
		       	'priority' => 50,
		       	'title' => __('Logo Alignment', 'wp-store-child'),
		       	'panel' => 'wp_store_header_setting'
				)
	   		);
		    ...
		    $wp_customize->add_setting(
		   	...
		   	$wp_customize->add_control(
			   	...
			   	array(
			      	'type' => 'radio',
			      	'label' => __('Choose the layout that you want', 'wp-store-child'),
			      	'section' => 'wp_store_header_settings_logo',
			      	'setting' => 'wp_store_header_settings_logo_alignment',
			      	'choices' => array(
				        'left'=>__('Left', 'wp-store-child'),
				        'center'=>__('Center', 'wp-store-child'),
				        'right'=>__('Right', 'wp-store-child')
			      		)
			   		)
			   	);
